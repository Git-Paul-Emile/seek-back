...
export class TokenService {
     ...
     */
    async createTokenPair(proprietaireId, telephone, role, userAgent, ipAddress) {
     ...
     */
    async refreshTokens(refreshTokenValue, userAgent, ipAddress) {
        try {
            ...
            if (storedToken.revokedAt) {
                throw new AppError("Token de rafraîchissement révoqué", StatusCodes.UNAUTHORIZED);
            }
            // Vérifier si le token a expiré
            if (new Date() > storedToken.expiresAt) {
                throw new AppError("Token de rafraîchissement expiré", StatusCodes.UNAUTHORIZED);
            }
            ...
            if (!proprietaire || proprietaire.statut !== "ACTIF") {
                throw new AppError("Compte non trouvé ou suspendu", StatusCodes.UNAUTHORIZED);
            }
            // Créer une nouvelle paire de tokens
            return this.createTokenPair(proprietaireId, proprietaire.telephone, proprietaire.role, userAgent, ipAddress);
        }
        catch (error) {
            if (error instanceof AppError) {
                throw error;
            }
            throw new AppError("Erreur lors du renouvellement du token", StatusCodes.UNAUTHORIZED);
        }
    }
    /**
     * Révoquer tous les tokens d'un utilisateur (déconnexion complète)
     */
    async revokeAllUserTokens(proprietaireId) {
     ...
     */
    async revokeToken(refreshTokenValue) {
        try {
            const payload = verifyRefreshToken(refreshTokenValue);
     ...
     */
    async detectCompromisedToken(proprietaireId, suspiciousIp) {
        // Révoquer tous les tokens de l'utilisateur
        await this.revokeAllUserTokens(proprietaireId);
        // Logger l'incident pour analyse
        console.error(`[SECURITY] Token compromis détecté pour l'utilisateur ${proprietaireId}. IP suspecte: ${suspiciousIp}`);
    }
}

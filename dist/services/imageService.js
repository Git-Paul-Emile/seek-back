 ...
 */
export const assessImageQuality = (metadata, isCover) => {
 ...
 */
export const generateUniqueFilename = (originalName, hash) => {
 ...
 */
export const processCoverImage = async (buffer, originalName) => {
    try {
 ...
 */
export const processAdditionalImage = async (buffer, originalName) => {
    try {
        // Validation du type MIME
        const imageInfo = await sharp(buffer).metadata();
        if (!imageInfo.format || !ALLOWED_FORMATS.includes(imageInfo.format)) {
 ...
 */
export const validateImageSet = (coverResult, additionalResults) => {
    const errors = [];

...
import { envoyerConfirmationPaiement, envoyerConfirmationPaiementProprietaire, envoyerPaiementLocataire, envoyerPreavisLocataire, envoyerPreavisProprietaire, envoyerResiliationLocataire, envoyerResiliationProprietaire, envoyerFinBailLocataire, envoyerPaiementEspecesLocataire, envoyerConfirmationEspecesProprietaire, } from "./notification.service.js";
...
function advanceByFrequence(date, frequence) {
...
function genererEcheances(params) {
    ...
    while (echeances.length < MAX) {
        if (dateFin && current > dateFin)
            break;
        if (dateMax && current > dateMax)
            break;
        const due = new Date(current);
        due.setHours(0, 0, 0, 0);
        const statut = due <= today ? "EN_ATTENTE" : "A_VENIR";
        echeances.push({
...
const msgProprietaire = (p) => prisma.messageInterne.create({ data: { proprietaireId: p.userId, bailId: p.bailId, bienId: p.bienId, titre: p.titre, corps: p.corps, type: p.type } });
const msgLocataire = (p) => prisma.messageInterneLocataire.create({ data: { locataireId: p.userId, bailId: p.bailId, bienId: p.bienId, titre: p.titre, corps: p.corps, type: p.type } });
const ensurePreviousUnpaidCleared = async (bailId, dateEcheance) => {
...
const getStatutId = async (slug) => {
    const statut = await BailRepo.getStatutBienBySlug(slug);
    if (!statut)
        throw new AppError(`Statut "${slug}" introuvable`, StatusCodes.INTERNAL_SERVER_ERROR);
    return statut.id;
};
/** Vérifie que le bien appartient au propriétaire */
const assertBienOwner = async (bienId, proprietaireId) => {
...
export const getBailActif = async (bienId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    return BailRepo.findActifByBien(bienId);
};
// ─── Historique des baux d'un bien ───────────────────────────────────────────
export const getHistoriqueBails = async (bienId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    return BailRepo.findByBien(bienId);
};
// ─── Créer un bail ────────────────────────────────────────────────────────────
export const creerBail = async (bienId, proprietaireId, data) => {
    ...
    if (existingBail) {
        throw new AppError("Ce bien a déjà un bail actif. Terminez-le avant d'en créer un nouveau.", StatusCodes.CONFLICT);
    }
...
export const mettreEnPreavis = async (bienId, bailId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    ...
    const updated = await prisma.bailLocation.update({
        where: { id: bailId },
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        data: { statut: "EN_PREAVIS", dateFinBail, initiePar: "PROPRIETAIRE" },
        include: {
...
export const mettreEnPreavisLocataire = async (locataireId, motif) => {
    // Trouver le bail actif du locataire
    const bail = await prisma.bailLocation.findFirst({
        where: { locataireId, statut: "ACTIF" },
    });
    if (!bail)
        throw new AppError("Aucun bail actif trouvé", StatusCodes.NOT_FOUND);
    ...
    const updatedLoc = await prisma.bailLocation.update({
        where: { id: bail.id },
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        data: { statut: "EN_PREAVIS", dateFinBail, initiePar: "LOCATAIRE", motifResiliation: motif },
        include: {
            locataire: { select: { id: true, nom: true, prenom: true, telephone: true, email: true, statut: true } },
            bien: { select: { id: true, titre: true, ville: true } },
        },
...
export const mettreEnRenouvellement = async (bienId, bailId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
    if (bail.bienId !== bienId)
        throw new AppError("Bail non associé à ce bien", StatusCodes.BAD_REQUEST);
    if (bail.statut !== "EN_PREAVIS")
        throw new AppError("Le bail doit être en préavis pour passer en renouvellement", StatusCodes.BAD_REQUEST);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return BailRepo.updateStatut(bailId, "EN_RENOUVELLEMENT");
};
// ─── Archiver un bail ─────────────────────────────────────────────────────────
export const archiverBail = async (bienId, bailId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const updated = await BailRepo.updateStatut(bailId, "ARCHIVE");
    // Locataire → ANCIEN
...
export const getBailAArchiver = async (bienId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    return BailRepo.findAArchiverByBien(bienId);
};
// ─── Terminer un bail ─────────────────────────────────────────────────────────
export const terminerBail = async (bienId, bailId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
    if (bail.bienId !== bienId)
        throw new AppError("Bail non associé à ce bien", StatusCodes.BAD_REQUEST);
    if (!["ACTIF", "EN_PREAVIS", "EN_RENOUVELLEMENT"].includes(bail.statut))
...
// ─── Résilier un bail ─────────────────────────────────────────────────────────
export const resilierBail = async (bienId, bailId, proprietaireId, motif) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
    if (bail.bienId !== bienId)
        throw new AppError("Bail non associé à ce bien", StatusCodes.BAD_REQUEST);
    if (!["ACTIF", "EN_PREAVIS"].includes(bail.statut))
...
export const resilierBailLocataire = async (locataireId, motif) => {
    const bail = await prisma.bailLocation.findFirst({
        where: { locataireId, statut: { in: ["ACTIF", "EN_PREAVIS"] } },
    });
    if (!bail)
        throw new AppError("Aucun bail actif ou en préavis trouvé", StatusCodes.NOT_FOUND);
...
export const annulerBail = async (bienId, bailId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
...
// ─── Prolonger un bail ────────────────────────────────────────────────────────
export const prolongerBail = async (bienId, bailId, proprietaireId, duree) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
    ...
    if (bail.statut === "EN_PREAVIS" || bail.statut === "EN_RENOUVELLEMENT") {
...
// ─── Échéancier ───────────────────────────────────────────────────────────────
export const getEcheancier = async (bienId, bailId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
    ...
    const isActiveForRenewal = ["ACTIF", "EN_PREAVIS", "EN_RENOUVELLEMENT"].includes(bail.statut);
    if (isActiveForRenewal && now.getMonth() >= 10) { // novembre = 10, décembre = 11
        ...
        if (dejaExistantes === 0) {
            ...
            if (lastCurrentYear) {
                const nextStart = advanceByFrequence(new Date(lastCurrentYear.dateEcheance), bail.frequencePaiement ?? "mensuel");
                const prochaineAnneeFin = new Date(anneeProchaine, 11, 31);
                const stopDate = bail.dateFinBail && new Date(bail.dateFinBail) < prochaineAnneeFin
...
export const payerEcheance = async (bienId, bailId, echeanceId, proprietaireId, data) => {
    await assertBienOwner(bienId, proprietaireId);
    ...
    if (echeance.bailId !== bailId)
        throw new AppError("Échéance non associée à ce bail", StatusCodes.BAD_REQUEST);
    ...
    let commissionTaux = null;
    let commissionMontant = null;
    let montantNet = null;
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const updated = await prisma.echeancierLoyer.update({
        where: { id: echeanceId },
        data: {
            statut: "PAYE",
...
export const payerMoisMultiples = async (bienId, bailId, proprietaireId, data) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
    ...
    await Promise.all(unpaid.map((item) => syncTransactionLoyer({
        proprietaireId,
        bienId,
        bailId,
        echeanceId: item.id,
...
export const payerEcheancesLocataire = async (locataireId, data) => {
    // Récupérer le bail actif du locataire
    const bail = await prisma.bailLocation.findFirst({
        where: { locataireId, statut: "ACTIF" },
    });
    if (!bail)
        throw new AppError("Aucun bail actif trouvé", StatusCodes.BAD_REQUEST);
 ...
 */
export const confirmerReceptionPaiement = async (bienId, bailId, echeanceId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const echeance = await prisma.echeancierLoyer.findUnique({ where: { id: echeanceId } });
    if (!echeance)
        throw new AppError("Échéance introuvable", StatusCodes.NOT_FOUND);
 ...
 */
export const enregistrerPaiementEspeces = async (bienId, bailId, echeanceId, proprietaireId, data) => {
    await assertBienOwner(bienId, proprietaireId);
    const echeance = await prisma.echeancierLoyer.findUnique({ where: { id: echeanceId } });
    if (!echeance)
        throw new AppError("Échéance introuvable", StatusCodes.NOT_FOUND);
    if (echeance.bailId !== bailId)
        throw new AppError("Échéance non associée à ce bail", StatusCodes.BAD_REQUEST);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if (["PAYE", "ANNULE", "EN_ATTENTE_CONFIRMATION"].includes(echeance.statut))
    ...
    await Promise.all(echeances.map((item) => syncTransactionLoyer({
        proprietaireId,
        bienId,
        bailId,
        echeanceId: item.id,
 ...
 */
export const confirmerPaiementEspacesLocataire = async (locataireId, echeanceId) => {
    const echeance = await prisma.echeancierLoyer.findUnique({ where: { id: echeanceId } });
    if (!echeance)
        throw new AppError("Échéance introuvable", StatusCodes.NOT_FOUND);
    if (echeance.locataireId !== locataireId)
        throw new AppError("Accès refusé", StatusCodes.FORBIDDEN);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if (echeance.statut !== "EN_ATTENTE_CONFIRMATION")
...
// ─── Caution ──────────────────────────────────────────────────────────────────
export const getCaution = async (bienId, bailId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    return prisma.depotCaution.findUnique({ where: { bailId } });
};
export const restituerCaution = async (bienId, bailId, proprietaireId, data) => {
    await assertBienOwner(bienId, proprietaireId);
    const depot = await prisma.depotCaution.findUnique({ where: { bailId } });
    if (!depot)
        throw new AppError("Dépôt de caution introuvable", StatusCodes.NOT_FOUND);
    const [edlEntree, edlSortie] = await Promise.all([
        prisma.etatDesLieux.findUnique({
            where: { bailId_type: { bailId, type: "ENTREE" } },
            select: { id: true, statut: true },
        }),
        prisma.etatDesLieux.findUnique({
            where: { bailId_type: { bailId, type: "SORTIE" } },
            select: { id: true, statut: true },
        }),
    ]);
    if (!edlEntree || edlEntree.statut !== "VALIDE" || !edlSortie || edlSortie.statut !== "VALIDE") {
        throw new AppError("La caution ne peut être restituée qu'après des états des lieux d'entrée et de sortie validés.", StatusCodes.BAD_REQUEST);
    }
...
const DEFAULT_MOBILE_MONEY = [
    {
        nom: "Mobile Money",
        instructions: "Contactez le propriétaire pour obtenir ses coordonnées de paiement Mobile Money.",
    },
];
export const getMobileMoney = async (bienId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const bien = await prisma.bien.findUnique({
        where: { id: bienId },
        select: { pays: true },
...
// ─── Solde locataire ──────────────────────────────────────────────────────────
export const getSolde = async (bienId, bailId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
...
export const prolongerEcheancesAnnee = async (bienId, bailId, proprietaireId, anneeActuelle) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
...
export const activerBail = async (bienId, bailId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const bail = await BailRepo.findById(bailId);
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
...
export const getEcheancierForLocataire = async (locataireId) => {
    const bail = await prisma.bailLocation.findFirst({
        where: { locataireId, statut: "ACTIF" },
    });
    if (!bail)
...
export const getContratForLocataire = async (locataireId) => {
    // Récupérer le bail actif du locataire
    const bail = await prisma.bailLocation.findFirst({
        where: { locataireId, statut: "ACTIF" },
        include: {
...
export const getProprietaireForLocataire = async (locataireId) => {
    // Récupérer le bail actif du locataire
    const bail = await prisma.bailLocation.findFirst({
        where: { locataireId, statut: "ACTIF" },
        include: {
...
export const getBiensAvecBailActif = async (proprietaireId) => {
...
export const getBiensEnRetard = async (proprietaireId) => {
    const now = new Date();
...
//# sourceMappingURL=bail.service.js.map
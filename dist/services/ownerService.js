...
import { proprietaireInscriptionSchema, proprietaireConnexionSchema, resetPasswordSchema } from "../validators/ownerValidator.js";
...
export class ProprietaireService {
     ...
     */
    async login(telephone, mot_de_passe, userAgent, ipAddress) {
        // Valider les données
        ...
        // Générer les tokens
        const { accessToken, refreshToken } = await tokenService.createTokenPair(proprietaire.id, proprietaire.telephone, proprietaire.role, userAgent, ipAddress);
        // Retourner sans le mot de passe
        const { mot_de_passe: _, ...proprietaireSansMotDePasse } = proprietaire;
        return {
            success: true,
     ...
     */
    async logout(proprietaireId, res) {
     ...
     */
    async getProfile(proprietaireId) {
     ...
     */
    async updateProfile(proprietaireId, data) {
     ...
     */
    async changePassword(proprietaireId, ancienMotDePasse, nouveauMotDePasse) {
        // Vérifier si le propriétaire existe
        const proprietaire = await proprietaireRepository.findById(proprietaireId);
        if (!proprietaire) {
            throw new AppError("Propriétaire non trouvé", StatusCodes.NOT_FOUND);
     ...
     */
    async forgotPasswordBySms(telephone) {
        // Anti-énumération
        const proprietaire = await proprietaireRepository.findByPhone(telephone);
        if (!proprietaire) {
            return {
     ...
     */
    async resetPasswordByEmail(code, mot_de_passe) {
        // Valider les données
     ...
     */
    async resetPasswordBySms(code, mot_de_passe) {
        const validationResult = resetPasswordSchema.safeParse({ code, mot_de_passe });
        if (!validationResult.success) {
            throw new AppError("Données invalides", StatusCodes.BAD_REQUEST);
        }
     ...
     */
    async deleteAccount(proprietaireId, res) {

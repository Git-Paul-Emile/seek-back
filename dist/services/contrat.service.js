...
const formatDuree = (debut, fin) => {
    if (!fin)
        return "indéterminée";
    const d = new Date(debut);
    const f = new Date(fin);
    const totalMonths = (f.getFullYear() - d.getFullYear()) * 12 + (f.getMonth() - d.getMonth());
    if (totalMonths <= 0)
        return "";
    if (totalMonths < 12)
...
export const substituerVariables = (contenu, data) => {
...
const getBailWithRelations = async (bailId, bienId) => {
 ...
 */
const findModele = async (typeBail) => {
    if (typeBail) {
...
// ─── Service functions ────────────────────────────────────────────────────────
export const getContrat = async (bienId, bailId, proprietaireId) => {
...
export const genererContrat = async (bienId, bailId, proprietaireId) => {
    ...
    if (!modele)
        throw new AppError("Aucun modèle de contrat disponible. Contactez l'administrateur.", StatusCodes.NOT_FOUND);
...
export const updateContrat = async (bienId, bailId, contratId, proprietaireId, contenu) => {
    await assertBienOwner(bienId, proprietaireId);
...
export const activerContrat = async (bienId, bailId, contratId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const contrat = await ContratRepo.findByBailId(bailId);
    if (!contrat || contrat.id !== contratId)
        throw new AppError("Contrat introuvable", StatusCodes.NOT_FOUND);
    ...
    if (bail.locataire && bail.locataire.statut !== "ACTIF") {
        try {
            ...
            console.warn("[activerContrat] Impossible de générer le lien d'activation:", error.message);
        }
    }
    return {
...
export const envoyerContrat = async (bienId, bailId, contratId, proprietaireId) => {
    await assertBienOwner(bienId, proprietaireId);
    const contrat = await ContratRepo.findByBailId(bailId);
    if (!contrat || contrat.id !== contratId)
        throw new AppError("Contrat introuvable", StatusCodes.NOT_FOUND);
    ...
    if (bail.locataire && bail.locataire.statut !== "ACTIF") {
        try {
            ...
            console.warn("[envoyerContrat] Impossible de générer le lien d'activation:", error.message);
        }
    }
    ...
    if (bail.locataire.email) {
        ...
        `,
            ...
            console.warn("[envoyerContrat] Échec de l'envoi email :", err.message);
        }
    }
    // Envoi SMS (best-effort — l'email riche est déjà envoyé par sendMail ci-dessus)
    try {
        const contenuSms = `Bonjour ${bail.locataire.prenom} ${bail.locataire.nom}, votre contrat de bail "${contrat.titre}" vous a été transmis par votre propriétaire. Connectez-vous à votre espace SEEK pour le consulter. - SEEK Immobilier`;
        ...
        console.warn("[envoyerContrat] Échec de l'envoi SMS :", err.message);
    }

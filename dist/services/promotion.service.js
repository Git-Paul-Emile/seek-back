 ...
 */
export const activatePromotion = async (bienId, proprietaireId, dureeJours = 7) => {
    ...
    if (proprietaire?.estRestreint) {
        const dateFin = proprietaire.dateFinRestriction
            ...
            : "sous peu";
        throw new AppError(`Votre compte est restreint. Mise en avant désactivée jusqu'au ${dateFin}.`, StatusCodes.FORBIDDEN);
    }
    const now = new Date();
 ...
 */
export const deactivatePromotion = async (bienId, proprietaireId) => {
    // Vérifier que le bien existe et appartient au propriétaire
    const bien = await prisma.bien.findFirst({
        where: {
            id: bienId,
 ...
 */
export const getAnnoncesMiseEnAvant = async (limit = CONFIG.NB_ANNOACES_AFFICHAGES) => {
    const now = new Date();
    ...
    const shouldRotate = promotedAnnonces.some((annonce) => {
        if (!annonce.dernierAffichage)
            return true;
        const diffMinutes = (now.getTime() - new Date(annonce.dernierAffichage).getTime()) / (1000 * 60);
    ...
    if (shouldRotate && promotedAnnonces.length > limit) {
        ...
        const sortedByDisplay = [...promotedAnnonces].sort((a, b) => {
            if (!a.dernierAffichage)
                return -1;
            if (!b.dernierAffichage)
                return 1;
            return (new Date(a.dernierAffichage).getTime() - new Date(b.dernierAffichage).getTime());
        });
        ...
        await Promise.all(rotatedAnnonces.map((annonce) => prisma.bien.update({
            where: { id: annonce.id },
        ...
        await Promise.all(newRotation.map((rotation) => rotation
            ? prisma.bien.update({
 ...
 */
export const getPromotionStatus = async (bienId, proprietaireId) => {
    const bien = await prisma.bien.findFirst({
        where: {
            id: bienId,
            proprietaireId,
    ...
    return {
        ...
        joursRestants: isActive && bien.dateFinPromotion
            ? Math.ceil((new Date(bien.dateFinPromotion).getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
            : 0,
    };
};
/**
 ...
 */
export const getPromotionStats = async (proprietaireId) => {
    const now = new Date();
 ...
 */
export const extendPromotion = async (bienId, proprietaireId, joursSupplementaires = 7) => {
    const bien = await prisma.bien.findFirst({
        where: {
            id: bienId,
            proprietaireId,

...
export const updateEtatDesLieux = async (id, proprietaireId, data) => {
    const edl = await EtatDesLieuxRepo.findById(id);
    if (!edl)
        throw new AppError("État des lieux introuvable", StatusCodes.NOT_FOUND);
    if (edl.proprietaireId !== proprietaireId)
...
export const getEtatDesLieuxList = async (bailId, userId, role) => {
    const bail = await prisma.bailLocation.findUnique({ where: { id: bailId } });
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
    if (role === "PROPRIETAIRE" && bail.proprietaireId !== userId)
        throw new AppError("Accès refusé", StatusCodes.FORBIDDEN);
    if (role === "LOCATAIRE" && bail.locataireId !== userId)
        throw new AppError("Accès refusé", StatusCodes.FORBIDDEN);
    return EtatDesLieuxRepo.findAllByBail(bailId);
};
export const getEtatDesLieuxById = async (id, userId, role) => {
    const edl = await EtatDesLieuxRepo.findById(id);
    if (!edl)
        throw new AppError("État des lieux introuvable", StatusCodes.NOT_FOUND);
    if (role === "PROPRIETAIRE" && edl.proprietaireId !== userId)
        throw new AppError("Accès refusé", StatusCodes.FORBIDDEN);
    if (role === "LOCATAIRE" && edl.locataireId !== userId)
        throw new AppError("Accès refusé", StatusCodes.FORBIDDEN);
    return edl;
};
export const submitForValidation = async (id, proprietaireId) => {
    const edl = await EtatDesLieuxRepo.findById(id);
    if (!edl)
        throw new AppError("État des lieux introuvable", StatusCodes.NOT_FOUND);
    if (edl.proprietaireId !== proprietaireId)
...
export const deleteEtatDesLieux = async (id, proprietaireId) => {
    const edl = await EtatDesLieuxRepo.findById(id);
    if (!edl)
        throw new AppError("État des lieux introuvable", StatusCodes.NOT_FOUND);
    if (edl.proprietaireId !== proprietaireId)
...
export const contesterElementsLocataire = async (id, locataireId, elements) => {
    const edl = await EtatDesLieuxRepo.findById(id);
    if (!edl)
        throw new AppError("État des lieux introuvable", StatusCodes.NOT_FOUND);
    if (edl.locataireId !== locataireId)
        throw new AppError("Accès refusé", StatusCodes.FORBIDDEN);
...
export const resoudreContestationsProprietaire = async (id, proprietaireId, resolutions) => {
    const edl = await EtatDesLieuxRepo.findById(id);
    if (!edl)
        throw new AppError("État des lieux introuvable", StatusCodes.NOT_FOUND);
    if (edl.proprietaireId !== proprietaireId)
...
export const validerEtatDesLieux = async (id, locataireId, documentPdfUrl) => {
    const edl = await EtatDesLieuxRepo.findById(id);
    if (!edl)
        throw new AppError("État des lieux introuvable", StatusCodes.NOT_FOUND);
    if (edl.locataireId !== locataireId)
...
export const getAllEtatsDesLieuxLocataire = async (locataireId) => {
    return EtatDesLieuxRepo.findAllByLocataire(locataireId);
};
export const getComparison = async (bailId, userId, role) => {
    const bail = await prisma.bailLocation.findUnique({ where: { id: bailId } });
    if (!bail)
        throw new AppError("Bail introuvable", StatusCodes.NOT_FOUND);
    if (role === "PROPRIETAIRE" && bail.proprietaireId !== userId)

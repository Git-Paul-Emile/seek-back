...
// ─── Calcul ───────────────────────────────────────────────────────────────────
export const computeScoreForProprietaire = async (proprietaireId) => {
    ...
    // ── Ancienneté en mois ────────────────────────────────────────────────────
    const now = new Date();
    const moisAnciennete = Math.floor((now.getTime() - proprietaire.createdAt.getTime()) / (1000 * 60 * 60 * 24 * 30));
    // ── Bails actifs ──────────────────────────────────────────────────────────
    ...
    if (proprietaire.statutVerification === "VERIFIED")
        badges.push("identite_verifiee");
    if (moisAnciennete >= 12)
        badges.push("anciennete_1an");
    return {
        total,
        details: {
            base,

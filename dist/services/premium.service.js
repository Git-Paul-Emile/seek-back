...
import { FORMULES_PREMIUM, } from "../types/premium.types.js";
 ...
 */
export const simulerPaiementEtActiverPromotion = async (bienId, proprietaireId, formuleId, modePaiement) => {
    ...
    if (bien.statutAnnonce !== "PUBLIE") {
        throw new AppError("Seules les annonces publiées peuvent être mises en avant", StatusCodes.BAD_REQUEST);
    }
 ...
 */
export const getFormulesPremium = async () => {
 ...
 */
export const getFormuleById = (formuleId) => {
...
export const adminUpdateFormule = async (id, data) => {
    return prisma.formulePremium.update({ where: { id }, data });
};
export const adminDeleteFormule = async (id) => {
 ...
 */
export const getDetailsTransaction = async (bienId, proprietaireId) => {
    const bien = await prisma.bien.findFirst({
        where: {
            id: bienId,
            proprietaireId,
    ...
    const now = new Date();
    const isActive = bien.estMisEnAvant &&
    ...
    return {
        ...
        joursRestants: isActive && bien.dateFinPromotion
            ? Math.ceil((new Date(bien.dateFinPromotion).getTime() - now.getTime()) /
                (1000 * 60 * 60 * 24))
            : 0,
    };
};
const savePromotionToHistory = async (params) => {
 ...
 */
export const arreterMiseEnAvant = async (bienId, proprietaireId) => {
    // 1. Vérifier que le bien existe et appartient au propriétaire
    const bien = await prisma.bien.findFirst({
        where: {
            id: bienId,
    ...
    if (!bien.estMisEnAvant || !bien.dateFinPromotion || !bien.dateDebutPromotion) {
        throw new AppError("Ce bien n'est pas actuellement en mise en avant", StatusCodes.BAD_REQUEST);
    }
    const now = new Date();
    const dateFinPrevue = new Date(bien.dateFinPromotion);
    // Vérifier si la promotion est déjà terminée
    if (dateFinPrevue <= now) {
        throw new AppError("Cette mise en avant est déjà terminée", StatusCodes.BAD_REQUEST);
    }
    // Calculer les jours restants
    const joursRestants = Math.ceil((dateFinPrevue.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
export const getHistoriqueMisesEnAvant = async (bienId, proprietaireId) => {
 ...
 */
export const getHistoriqueMisesEnAvantProprietaire = async (proprietaireId, options = {}) => {

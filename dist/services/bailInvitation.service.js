...
export const creerInvitation = async (proprietaireId, data) => {
    ...
    const invExistante = await InvitationRepo.findPendingByBienAndLocataire(data.bienId, data.locataireId);
    if (invExistante)
        throw new AppError("Une invitation est déjà en attente pour ce locataire sur ce bien.", StatusCodes.CONFLICT);
    ...
    const contenu = `Un propriétaire vous a envoyé une invitation de bail pour ${bienLabel}. ` +
...
export const getInvitationsByLocataire = async (locataireId) => {
    await InvitationRepo.expireOldInvitations();
    return InvitationRepo.findByLocataireId(locataireId);
};
// ─── Accepter une invitation ─────────────────────────────────────────────────
export const accepterInvitation = async (token, locataireId) => {
    ...
    if (bailExistant)
        throw new AppError("Ce bien a déjà un bail actif.", StatusCodes.CONFLICT);
...
export const refuserInvitation = async (token, locataireId) => {
    const invitation = await InvitationRepo.findByToken(token);
    if (!invitation)
        throw new AppError("Invitation introuvable", StatusCodes.NOT_FOUND);
    if (invitation.locataireId !== locataireId)
...
//# sourceMappingURL=bailInvitation.service.js.map
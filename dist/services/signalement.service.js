...
// -- Récupérer liste
export const getSignalements = async (filter) => {
...
export const createSignalement = async (data) => {
    ...
    const existing = await SignalementRepo.getExisting(data.bienId, data.comptePublicId ?? undefined, data.telephone);
    if (existing) {
...
// -- Valider (Admin)
export const validerSignalement = async (id, adminId) => {
    const signalement = await SignalementRepo.findById(id);
    if (!signalement) {
        throw new AppError("Signalement introuvable", StatusCodes.NOT_FOUND);
    }
    ...
    const result = await SignalementRepo.valider(id, adminId, bien.id, bien.proprietaire.id);
    // SMS Propriétaire
    ...
    if (result.proprietaire.nbSignalementsValides >= 3 && !result.proprietaire.estSuspendu) {
        try {
            await SuspenionService.suspendreProprietaire(result.proprietaire.id, { motif: `Bannissement automatique : ${result.proprietaire.nbSignalementsValides} signalements validés` }, adminId, true // masquerAnnonces = true
            );
...
// -- Rejeter (Admin)
export const rejeterSignalement = async (id, adminId) => {
    const signalement = await SignalementRepo.findById(id);
    if (!signalement) {
        throw new AppError("Signalement introuvable", StatusCodes.NOT_FOUND);
    }

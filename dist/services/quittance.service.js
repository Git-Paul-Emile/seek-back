...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const assertOwnerBien = async (bienId, proprietaireId) => {
...
export const getQuittance = async (bienId, bailId, echeanceId, proprietaireId) => {
...
export const genererQuittance = async (bienId, bailId, echeanceId, proprietaireId) => {
    await assertOwnerBien(bienId, proprietaireId);
    const echeance = await prisma.echeancierLoyer.findUnique({
        where: { id: echeanceId },
        include: { quittance: true },
    ...
    if (echeance.quittance)
        return echeance.quittance;
...
export const getQuittancesBail = async (bienId, bailId, proprietaireId) => {
    await assertOwnerBien(bienId, proprietaireId);
 ...
 */
export const genererQuittanceInterne = async (echeanceId) => {
    const echeance = await prisma.echeancierLoyer.findUnique({
        where: { id: echeanceId },
        include: {
            quittance: true,
...
export const getQuittancesLocataire = async (locataireId) => prisma.quittance.findMany({
    where: { locataireId },
    include: {
        echeance: {
            select: {
...
//# sourceMappingURL=quittance.service.js.map
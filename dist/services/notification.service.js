...
async function dispatchSmsEtEmail(params) {
...
export const envoyerNotification = async (payload) => {
    ...
    if (!payload.noSmsEmail) {
        enqueueNotif(() => dispatchSmsEtEmail({
...
export const envoyerRappelLoyer = async (params) => {
    const contenu = `Bonjour ${params.locataireNom}, votre loyer de ${params.montant.toLocaleString("fr-FR")} FCFA` +
...
export const envoyerConfirmationPaiement = async (params) => {
    const contenu = `Bonjour ${params.locataireNom}, votre paiement de ${params.montant.toLocaleString("fr-FR")} FCFA` +
...
export const envoyerInitiationPaiement = async (params) => {
    const contenu = `Votre locataire ${params.locataireNom} indique avoir initié le paiement de ` +
...
export const envoyerPaiementLocataire = async (params) => {
    ...
    const contenu = `Votre locataire ${params.locataireNom} a enregistré un paiement de loyer de ` +
...
export const envoyerConfirmationEspecesProprietaire = async (params) => {
    const contenu = `Votre locataire ${params.locataireNom} a confirmé son paiement en espèces` +
...
export const envoyerPreavisLocataire = async (params) => {
    ...
    const contenu = `Bonjour ${params.locataireNom}, votre bailleur a déclenché un préavis` +
        (params.bienTitre ? ` pour ${params.bienTitre}` : "") +
...
export const envoyerPreavisProprietaire = async (params) => {
    const fin = new Date(params.dateFinBail).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" });
    const contenu = `Votre locataire ${params.locataireNom} a donné son préavis` +
        (params.bienTitre ? ` pour ${params.bienTitre}` : "") +
        `. Date de fin de bail fixée au ${fin}. - SEEK Immobilier`;
    return envoyerNotification({
        type: "PREAVIS",
...
export const envoyerResiliationLocataire = async (params) => {
    const contenu = `Bonjour ${params.locataireNom}, votre bailleur a résilié votre bail` +
        (params.bienTitre ? ` pour ${params.bienTitre}` : "") +
...
export const envoyerResiliationProprietaire = async (params) => {
    const contenu = `Votre locataire ${params.locataireNom} a résilié le bail` +
        (params.bienTitre ? ` pour ${params.bienTitre}` : "") +
        (params.motif ? `. Motif : ${params.motif}` : "") +
        `. - SEEK Immobilier`;
    return envoyerNotification({
...
export const envoyerFinBailLocataire = async (params) => {
    const contenu = `Bonjour ${params.locataireNom}, votre bail a été clôturé par votre bailleur` +
        (params.bienTitre ? ` pour ${params.bienTitre}` : "") +
...
export const envoyerLienActivationLocataire = async (params) => {
    const contenu = `Bonjour ${params.locataireNom}, votre propriétaire vous invite à activer votre compte locataire SEEK. ` +
...
export const envoyerResetPasswordProprietaire = async (params) => {
    const contenu = `Bonjour ${params.prenom}, réinitialisez votre mot de passe SEEK : ${params.lien} (valable 1h). ` +
...
export const envoyerOtpTelephone = async (params) => {
    const contenu = `Bonjour ${params.prenom}, votre code de vérification SEEK est : ${params.otp}. ` +
...
export const getNotificationsBail = async (bailId) => prisma.notification.findMany({
    where: { bailId },
    orderBy: { createdAt: "desc" },
});
export const getNotificationsEcheance = async (echeanceId) => prisma.notification.findMany({
    where: { echeanceId },
    orderBy: { createdAt: "desc" },
});

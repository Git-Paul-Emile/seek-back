...
const INSTRUCTIONS_PAR_PROVIDER = {
    "Orange Money": (p) => `Composez *144# sur votre téléphone Orange → Transfert d'argent → ` +
    ...
    "MTN Mobile Money": (p) => `Composez *126# ou *133# → MoMo → Envoyer → ` +
        `Numéro : ${p.telephoneBeneficiaire} → Montant : ${p.montant.toLocaleString("fr-FR")} FCFA → ` +
        `Référence : ${p.reference}. Transmettez la confirmation à votre propriétaire.`,
    "Wave": (p) => `Ouvrez l'application Wave → Envoyer → Numéro : ${p.telephoneBeneficiaire} → ` +
        `Montant : ${p.montant.toLocaleString("fr-FR")} FCFA → Note : ${p.reference}. ` +
        `Transmettez la capture d'écran à votre propriétaire.`,
    "Moov Money": (p) => `Composez *555# → Transfert → Numéro : ${p.telephoneBeneficiaire} → ` +
        `Montant : ${p.montant.toLocaleString("fr-FR")} FCFA → Référence : ${p.reference}.`,
    "Free Money": (p) => `Composez *555# (Free) → Envoi → Numéro : ${p.telephoneBeneficiaire} → ` +
        `Montant : ${p.montant.toLocaleString("fr-FR")} FCFA → Référence : ${p.reference}.`,
    "Airtel Money": (p) => `Ouvrez l'application Airtel Money → Envoyer → ` +
        `Numéro : ${p.telephoneBeneficiaire} → Montant : ${p.montant.toLocaleString("fr-FR")} FCFA → ` +
        `Référence : ${p.reference}.`,
    "T-Money": (p) => `Composez *145# (T-Money Togocel) → Transfert → Numéro : ${p.telephoneBeneficiaire} → ` +
        `Montant : ${p.montant.toLocaleString("fr-FR")} FCFA → Référence : ${p.reference}.`,
    "Flooz": (p) => `Composez *155# (Flooz Moov Togo) → Transfert → Numéro : ${p.telephoneBeneficiaire} → ` +
        `Montant : ${p.montant.toLocaleString("fr-FR")} FCFA → Référence : ${p.reference}.`,
};
const getInstructions = (p) => {
...
export const initierPaiementMobileMoney = async (params) => {
    ...
    return {
        ...
        stub: true,
        instructions,
        message: `Intégration ${params.provider} en cours de déploiement. ` +
            `Suivez les instructions ci-dessus pour effectuer le paiement manuellement.`,
    };
};
// ─── Vérifier le statut d'un paiement (STUB) ──────────────────────────────────
export const verifierStatutPaiement = async (_transactionId, _provider) => {
    // TODO: Remplacer par GET /transactions/{id} de l'API du provider
    return {
        statut: "EN_COURS",
        stub: true,
        message: "Vérification automatique non disponible. " +

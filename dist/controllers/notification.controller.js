...
export const envoyerRappel = async (req, res) => {
    ...
    }); // cast to any so TypeScript knows bail property exists
    if (!echeance)
        throw new AppError("Échéance introuvable", StatusCodes.NOT_FOUND);
    if (echeance.bailId !== bailId)
        throw new AppError("Données incohérentes", StatusCodes.BAD_REQUEST);
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Rappel envoyé", data: result }));
};
// ─── Lister l'historique des rappels d'une échéance ──────────────────────────
/** GET /api/biens/:id/bail/:bailId/echeancier/:echeanceId/rappels */
export const getRappelsEcheance = async (req, res) => {
    const proprietaireId = getOwner(req);
    const { id: bienId, echeanceId } = req.params;
    const bien = await prisma.bien.findUnique({ where: { id: bienId }, select: { proprietaireId: true } });
    if (!bien)
        throw new AppError("Bien introuvable", StatusCodes.NOT_FOUND);
    if (bien.proprietaireId !== proprietaireId)
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Historique des relances récupéré", data: rappels }));
};
// ─── Lister les notifications d'un bail ───────────────────────────────────────
/** GET /api/biens/:id/bail/:bailId/notifications */
export const getNotificationsBail = async (req, res) => {
    const proprietaireId = getOwner(req);
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Notifications récupérées", data: notifications }));
};

...
export const register = async (req, res) => {
    ...
    if (!nom || !prenom || !telephone || !password) {
        res.status(StatusCodes.BAD_REQUEST).json(jsonResponse({ status: "fail", message: "nom, prenom, telephone et password sont requis" }));
        return;
    }
    ...
    res.status(StatusCodes.CREATED).json(jsonResponse({ status: "success", message: "Compte créé avec succès", data: compte }));
};
// POST /api/public/auth/login
export const login = async (req, res) => {
    const { telephone, password } = req.body;
    if (!telephone || !password) {
        res.status(StatusCodes.BAD_REQUEST).json(jsonResponse({ status: "fail", message: "telephone et password sont requis" }));
        return;
    }
...
export const updateProfile = async (req, res) => {
    ...
    if (!nom && !prenom && email === undefined) {
        res.status(StatusCodes.BAD_REQUEST).json(jsonResponse({ status: "fail", message: "Aucun champ à mettre à jour" }));
        return;
    }
...
export const changePassword = async (req, res) => {
    ...
    if (!currentPassword || !newPassword) {
        res.status(StatusCodes.BAD_REQUEST).json(jsonResponse({ status: "fail", message: "currentPassword et newPassword sont requis" }));
        return;
    }
    if (newPassword.length < 6) {
        res.status(StatusCodes.BAD_REQUEST).json(jsonResponse({ status: "fail", message: "Le nouveau mot de passe doit faire au moins 6 caractères" }));
        return;
    }
...
export const deleteAccount = async (req, res) => {
    ...
    if (!password) {
        res.status(StatusCodes.BAD_REQUEST).json(jsonResponse({ status: "fail", message: "Mot de passe requis pour confirmer la suppression" }));
        return;
    }

...
export const activatePromotion = async (req, res) => {
    ...
    const duree = dureeJours ? parseInt(dureeJours) : 7;
...
export const extendPromotion = async (req, res) => {
    const proprietaireId = req.owner?.id;
    if (!proprietaireId) {
        throw new AppError("Authentification requise", StatusCodes.UNAUTHORIZED);
    }
    ...
    const jours = joursSupplementaires ? parseInt(joursSupplementaires) : 7;
...
export const getAdminHistoriquePromotions = async (req, res) => {
...
export const getAdminStatsPromotions = async (_req, res) => {
    const { prisma } = await import("../config/database.js");
    const now = new Date();
...
export const getAnnoncesMiseEnAvant = async (req, res) => {

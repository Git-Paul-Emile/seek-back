...
export const getAll = async (req, res) => {
    ...
    const typeBail = req.query.typeBail;
    const actifStr = req.query.actif;
...
export const getActifs = async (req, res) => {
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Modèles actifs récupérés", data: items }));
};
// ─── Create ──────────────────────────────────────────────────────────────────
export const create = async (req, res) => {
    const { titre, typeBail, contenu, actif, ordre } = req.body;
    if (!titre || !contenu) {
        res.status(StatusCodes.BAD_REQUEST).json(jsonResponse({ status: "fail", message: "titre et contenu sont requis" }));
        return;
    }
    const modele = await ModeleContratService.create({ titre, typeBail: typeBail ?? null, contenu, actif, ordre });
    res.status(StatusCodes.CREATED).json(jsonResponse({ status: "success", message: "Modèle créé", data: modele }));
};
// ─── Update ──────────────────────────────────────────────────────────────────
export const update = async (req, res) => {
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Modèle mis à jour", data: modele }));
};
// ─── Delete ──────────────────────────────────────────────────────────────────
export const remove = async (req, res) => {
    await ModeleContratService.remove(req.params.id);
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Modèle supprimé" }));
};
//# sourceMappingURL=modeleContrat.controller.js.map
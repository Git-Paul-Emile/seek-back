...
export const forgotPassword = async (req, res) => {
    ...
    try {
        const { token, email, telephone, prenom, proprietaireId } = await OwnerService.requestPasswordReset(identifiant);
...
export const getMessagesInternes = async (req, res) => {
...
export const marquerMessagesLus = async (req, res) => {
    const proprietaireId = req.owner?.id;
    if (!proprietaireId) {
        res.status(StatusCodes.UNAUTHORIZED).json(jsonResponse({ status: "fail", message: "Non authentifié" }));
        return;
...
export const renvoyerOtp = async (req, res) => {
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Code OTP renvoyé sur votre téléphone." }));
};
// ─── POST /api/owner/auth/verifier-telephone-public ─────────────────────────
export const verifierTelephonePublic = async (req, res) => {
...
export const renvoyerOtpPublic = async (req, res) => {

...
export const listDocuments = async (req, res) => {
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Documents récupérés", data: documents }));
};
...
export const uploadDocument = async (req, res) => {
    ...
    const file = req.file;
    const { type } = req.body;
    const nom = req.body.nom || file?.originalname || "document";
    if (!file) {
    ...
    res.status(StatusCodes.CREATED).json(jsonResponse({ status: "success", message: "Document uploadé", data: doc }));
};
...
export const deleteDocument = async (req, res) => {
    ...
    if (!doc)
        throw new AppError("Document introuvable ou non autorisé", StatusCodes.NOT_FOUND);
...
//# sourceMappingURL=documentBien.controller.js.map
...
export const annulerAnnonce = async (req, res) => {
    ...
    if (!proprietaireId)
        throw new AppError("Authentification requise", StatusCodes.UNAUTHORIZED);
    await BienService.annulerAnnonce(req.params.id, proprietaireId);
    res.status(StatusCodes.NO_CONTENT).send();
};
...
export const getDernieresAnnonces = async (req, res) => {
    ...
    const promotedIds = new Set(promotedAnnonces.map((a) => a.id));
    const filteredRegular = regularAnnonces.filter((a) => !promotedIds.has(a.id));
...
export const getDistinctLieux = async (_req, res) => {
    const lieux = await BienService.getDistinctLieux();
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Lieux récupérés", data: lieux }));
};
// ─── Public - recherche avec filtres ──────────────────────────────────────────
export const searchAnnoncesPubliques = async (req, res) => {
    const { ville, quartier, typeLogement, typeTransaction, prixMin, prixMax, chambres, surfaceMin, surfaceMax, meuble, parking, ascenseur, fumeurs, animaux, equipementIds, sortBy, sortOrder, page, limit, lat, lng, radius, } = req.query;
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Résultats de recherche", data: result }));
};
...
export const getAnnoncePublie = async (req, res) => {
    ...
    const ip = req.headers["x-forwarded-for"]?.split(",")[0]?.trim()
        ...
        ?? "unknown";
    const userId = req.comptePublic?.id ?? null;
    (async () => {
        try {
...
export const getStatsVuesBienController = async (req, res) => {
    const proprietaireId = req.owner?.id;
    if (!proprietaireId)
        throw new AppError("Authentification requise", StatusCodes.UNAUTHORIZED);
    const stats = await getStatsVuesBien(req.params.id, proprietaireId);
...
export const getStatsVuesOwnerController = async (req, res) => {
    const proprietaireId = req.owner?.id;
    if (!proprietaireId)
        throw new AppError("Authentification requise", StatusCodes.UNAUTHORIZED);
...
// ─── Stats vues admin ─────────────────────────────────────────────────────────
export const getAdminStatsVuesController = async (_req, res) => {
...
export const getStatsFavorisOwnerController = async (req, res) => {
    const proprietaireId = req.owner?.id;
    if (!proprietaireId) {
        throw new AppError("Authentification requise", StatusCodes.UNAUTHORIZED);
    }
...
export const getStatsFavorisBienController = async (req, res) => {
    const proprietaireId = req.owner?.id;
    if (!proprietaireId) {
        throw new AppError("Authentification requise", StatusCodes.UNAUTHORIZED);
    }
    const stats = await getStatsFavorisBien(req.params.id, proprietaireId);
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Stats favoris récupérées", data: stats }));
};

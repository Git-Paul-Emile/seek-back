 ...
 */
router.post("/login", limiteurAuth, validate(loginSchema), controllerWrapper(AuthController.login));
/**
 * POST /api/auth/refresh
 * Rate limit : 30 requêtes / heure
 * Le cookie refreshToken n'est envoyé que sur ce path exact
 */
router.post("/refresh", limiteurRefresh, controllerWrapper(AuthController.refresh));
/**
 ...
 */
router.put("/change-password", authenticate, validate(changePasswordSchema), controllerWrapper(AuthController.changePassword));

 ...
 */
router.get("/accueil", controllerWrapper(PromotionController.getAnnoncesMiseEnAvant));
/**
 ...
 */
router.get("/stats", authenticateOwner, controllerWrapper(PromotionController.getPromotionStats));
/**
 * POST /api/promotions/:id/activer - Activer la mise en avant d'une annonce
 * Body: { dureeJours?: number } - Durée en jours (1-90, défaut 7)
 */
router.post("/:id/activer", authenticateOwner, validateId, controllerWrapper(PromotionController.activatePromotion));
/**
 * POST /api/promotions/:id/desactiver - Désactiver la mise en avant d'une annonce
 */
router.post("/:id/desactiver", authenticateOwner, validateId, controllerWrapper(PromotionController.deactivatePromotion));
/**
 * GET /api/promotions/:id/statut - Récupérer le statut de mise en avant d'une annonce
 */
router.get("/:id/statut", authenticateOwner, validateId, controllerWrapper(PromotionController.getPromotionStatus));
/**
 ...
 */
router.post("/:id/prolonger", authenticateOwner, validateId, controllerWrapper(PromotionController.extendPromotion));

...
router.post("/upload", authenticateOwner, uploadImageMiddleware, controllerWrapper(async (req, res) => {
    if (!req.file) {
...
router.get("/", authenticateOwner, controllerWrapper(async (req, res) => {
...
router.post("/", authenticateOwner, controllerWrapper(async (req, res) => {
    const proprietaireId = req.owner.id;
    const { typePiece, pieceIdentiteRecto, pieceIdentiteVerso, selfie, conditionsAcceptees } = req.body;
    if (!typePiece) {
        return res.status(400).json({
            status: "error",
...
router.delete("/", authenticateOwner, controllerWrapper(async (req, res) => {
    const proprietaireId = req.owner.id;

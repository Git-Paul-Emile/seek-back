...
import * as Service from "../services/categorieChamp.service.js";
...
export const create = async (req: Request, res: Response): Promise<void> => {
  const item = await Service.create({ nom: req.body.nom, ordre: req.body.ordre });
  res.status(StatusCodes.CREATED).json(jsonResponse({ status: "success", message: "Catégorie créée", data: item }));
};

export const update = async (req: Request, res: Response): Promise<void> => {
  const item = await Service.update(req.params.id as string, { nom: req.body.nom, ordre: req.body.ordre });
